...
public class ValidacaoAluno {
	...
	public static LocalTime validaHorario(Scanner sc) {
		...
		do {
			System.out.println("Informe o horário desejado das 5h às 23h(formato HH:mm): ");
	...
	public static void verificarPersonalDisponivel(List<PersonalTrainer> listaPersonal, LocalTime horario) {
		...
		for(int i = 0; i < listaPersonal.size(); i++) {
			if(listaPersonal.get(i).getInicioAtendimento().isBefore(horario) && listaPersonal.get(i).getFimAtendimento().isAfter(horario)) {
				...
				System.out.println("Não há listaPersonal disponíveis nesse horario");
			}
		}
	}
	...
	public static PersonalTrainer selecionaPersonal(List<PersonalTrainer> listaPersonal, Scanner sc) {
		...
		do {
			...
			for (int i = 0; i < listaPersonal.size(); i++) {
				if (listaPersonal.get(i) instanceof PersonalTrainer && listaPersonal.get(i).getCref().equals(crefSelecionado)) {
	...
	public static void removeAgendamento(Scanner sc, List<Agendamento> agenda) {
		...
		do {
			...
			System.out.println(dataDeletada);
			
			System.out.println("Insira o horario do agendamento que deseja cancelar?");
			horarioInformado = sc.next();
			horarioDeletado = LocalTime.parse(horarioInformado);
			System.out.println(horarioDeletado);
			
			for (int i = 0; i < agenda.size(); i++) {
				if (agenda.get(i).getHorario().equals(horarioDeletado) && agenda.get(i).getData().equals(dataDeletada)) {
